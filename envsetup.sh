...
Arguments:
	...
	--qt <Qt PATH>					; set qt android path
...
# ndk path
	if [ -z "$ANDROID_NDK_ROOT" ] ; then
		[ -d /opt/android-ndk ] && ANDROID_NDK_ROOT=/opt/android-ndk
		[ -d ~/Android/Sdk/ndk-bundle ] && ANDROID_NDK_ROOT=$(realpath ~/Android/Sdk/ndk-bundle)
	fi
	if [ -z "$ANDROID_NDK_ROOT" ] ; then
		echo "Enter Android ndk path:"
		read ANDROID_NDK_ROOT
...
# sdk path
	if [ -z "$ANDROID_SDK_ROOT" ] ; then
		[ -d /opt/android-sdk-linux ] && ANDROID_SDK_ROOT=/opt/android-sdk-linux
		[ -d /opt/android-sdk ] && ANDROID_SDK_ROOT=/opt/android-sdk
		[ -d ~/Android/Sdk ] && ANDROID_SDK_ROOT=$(realpath ~/Android/Sdk)
	fi
	if [ -z "$ANDROID_SDK_ROOT" ] ; then
...
function setpath()
{
	...
	if [ -n "$ANDROID_HOME" -o -n "$ANDROID_NDK_ROOT" ]; then
		export PATH="$ANDROID_HOME/tools:$ANDROID_NDK_ROOT/:$PATH"
	fi
}
...
# set Qt path
[ "$NEED_QT" = "false"] || set_qtpath

# set android source path
[ "$NEED_SRC" = "false" ] || set_androidsrcpath

if [ "$NEED_NDK" != "false" -a -z "$ANDROID_NDK_ROOT" ] || [ "$NEED_SDK" != "false" -a -z "$ANDROID_SDK_ROOT" ] || [ "$NEED_QT" != "false" -a -z "$QT_DIR_PATH" ]; then
	warn_msg "ANDROID_NDK and ANDROID_SDK is not set."
	warn_msg "Set it to NDK and SDK directories."
	warn_msg "Qt directory is not set."
	usage
	unset_var
	return 0
fi
